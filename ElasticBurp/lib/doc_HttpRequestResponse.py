...
identifierAnalyzer = analyzer("identifier", tokenizer="keyword", filter=["lowercase"])
...
class DocHTTPRequestResponse(Document):
    ...
    request = Object(
        properties={
            ...
            "headernames": Text(
                analyzer=identifierAnalyzer, multi=True, fields={"keyword": Keyword()}
            ),
            "headers": Nested(
                properties={
                    "name": Text(
                        analyzer=identifierAnalyzer, fields={"keyword": Keyword()}
                    ),
            ...
            "body": Text(),
            "full": Text(fields={"keyword": Keyword()}),
        }
    )
    response = Object(
        properties={
            ...
            "extrefs": Text(
                multi=True, fields={"keyword": Keyword()}
    ...
    def save(self, storeResponseBody=True, **kwargs):
        ...
        if self.response.body and (
            (
                ...
                and self.response.inferred_content_type == "HTML"
            )
            or (
