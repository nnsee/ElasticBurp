...
def query_missingheader(s, headername, methods=None, responsecodes=None, invert=False):
    s = query_missing(
        s, "response.headernames", headername, methods, responsecodes, invert
    )
    return s
...
def query_missingparam(s, paramname, methods=None, responsecodes=None, invert=False):
    s = query_missing(
        s, "request.parameternames", paramname, methods, responsecodes, invert
    )
    return s
...
def query_vals(s, field, name, values, invert):
    ...
    if values:
        q = Q(
            "nested",
            path=field,
            query=Q("wildcard", **{field + ".value.keyword": values}),
        )
        if invert:
            s.query = ~q
        else:
    ...
    # 5. aggregate URLs
    s.aggs.bucket("field", "nested", path=field).bucket(
        "valuefilter", "filter", Q("match", **{field + ".name": name})
    ).bucket("values", "terms", field=field + ".value.keyword", size=args.size).bucket(
        "main", "reverse_nested"
    ).bucket(
        "urls", "terms", field="request.url.keyword", size=args.size
    )
    return s
...
argparser.add_argument(
    "--server", "-s", default="localhost", help="ElasticSearch server"
)
argparser.add_argument(
    "--index", "-i", default="wase-*", help="ElasticSearch index pattern to query"
)
argparser.add_argument(
    "--size",
    "-S",
    default=10000,
    type=int,
    help="Maximum number of results of aggregation (default: %(default)s)",
)
argparser.add_argument(
    "--field",
    "-f",
    action="append",
    help="Add fields to output. Prints full result instead of aggregated URLs.",
)
argparser.add_argument(
    "--domain",
    "-d",
    action="append",
    help="Restrict search to domain. Wildcards allowed. Can be used multiple times.",
)
...
argparser_missingheader = subargparsers.add_parser(
    "missingheader", help="Search for URLs which responses are missing a header"
)
argparser_missingheader.add_argument("header", help="Name of the header")
argparser_missingheader.add_argument(
    "--invert",
    "-i",
    action="store_true",
    help="Invert result, list all URLs where header is set",
)
argparser_missingheader.add_argument(
    "--method", "-m", action="append", help="Restrict search to given methods"
)
argparser_missingheader.add_argument(
    "--responsecode",
    "-c",
    action="append",
    help="Restrict search to responses with the given codes. Can be a single code (e.g. 200), a range (200-299) or wildcard (2*)",
)

argparser_missingparam = subargparsers.add_parser(
    "missingparameter",
    help="Search for URLs where the requests are missing a parameter with the given name",
)
argparser_missingparam.add_argument("parameter", help="Name of parameter to search")
argparser_missingparam.add_argument(
    "--invert",
    "-i",
    action="store_true",
    help="Invert result, list all URLs where header is set",
...
argparser_responseheadervals = subargparsers.add_parser(
    "responseheadervalues",
    help="Show all response header values and the URLs where the value was set",
)
argparser_responseheadervals.add_argument(
    "--urls", "-u", action="store_true", help="List URLs where header value is set"
)
argparser_responseheadervals.add_argument(
    "--max-urls", "-n", type=int, default=0, help="Maximum number of listed URLs"
)
argparser_responseheadervals.add_argument(
    "--values",
    "-v",
    help="Restrict to values matching the given pattern (wildcards allowed)",
)
argparser_responseheadervals.add_argument(
    "--invert", "-i", action="store_true", help="Invert values search"
)
argparser_responseheadervals.add_argument("header", help="Name of the response header")

argparser_requestheadervals = subargparsers.add_parser(
    "requestheadervalues",
    help="Show all request header values and the URLs where the value was set",
)
argparser_requestheadervals.add_argument(
    "--urls", "-u", action="store_true", help="List URLs where header value is set"
)
argparser_requestheadervals.add_argument(
    "--max-urls", "-n", type=int, default=0, help="Maximum number of listed URLs"
...
argparser_cookievals = subargparsers.add_parser(
    "cookievalues", help="Show all cookie values and the URLs where the value was set"
)
argparser_cookievals.add_argument(
    "--urls", "-u", action="store_true", help="List URLs where header value is set"
)
argparser_cookievals.add_argument(
    "--max-urls", "-n", type=int, default=0, help="Maximum number of listed URLs"
...
argparser_paramvals = subargparsers.add_parser(
    "parametervalues",
    help="Show all request parameter values and the URLs where the value was set",
)
argparser_paramvals.add_argument(
    "--urls", "-u", action="store_true", help="List URLs where parameter value is set"
)
argparser_paramvals.add_argument(
    "--max-urls", "-n", type=int, default=0, help="Maximum number of listed URLs"
)
...
if args.cmd == "missingheader":
    ...
    s = query_missingparam(
        s, args.parameter, args.method, args.responsecode, args.invert
    )
    querytype = QUERY_SEARCH
